...
def open_gfs_dataset(path: Path) -> xr.Dataset:
    """Open a GRIB2 file and merge all cfgrib groups into a single dataset."""

    # Avoid stale on-disk index files that can trigger warnings when GRIB files are updated.
    datasets = xr.backends.cfgrib.open_datasets(path, indexpath="")
    if not datasets:
        raise ValueError(f"No datasets found in GRIB file {path}")
